...
const TradeMaster = () => {
  ...
  // Generate level nodes for the selected world
  ...
  // Lesson content based on the selected world
  const getLessonContent = () => {
    if (selectedWorld === 'basics') {
      return {
        intro: "Trading is the process of buying and selling financial instruments like stocks, currencies, or commodities to make a profit. Before diving deep, let's understand some basic concepts.",
        steps: [
          {
            type: "text",
            content: "Markets are where trades happen. Common markets include stock exchanges, forex markets, and cryptocurrency exchanges."
          },
          {
            type: "quiz",
            question: "Which of these is NOT a common financial market?",
            options: ["Stock Exchange", "Forex Market", "Trading Simulator", "Cryptocurrency Exchange"],
            correctAnswer: 2
          },
          {
            type: "text",
            content: "Assets are what you trade. These can be stocks, currencies, commodities, cryptocurrencies, and more."
          },
          {
            type: "interactive",
        ...
        summary: "Great job! You've learned the very basics of trading. Remember that understanding markets and assets is the foundation of becoming a successful trader."
      };
    } else if (selectedWorld === 'technical') {
      return {
        intro: "Technical analysis involves studying price charts and patterns to predict future price movements.",
        steps: [
          {
            type: "text",
            content: "Charts are visual representations of price movements over time. Common chart types include line charts, bar charts, and candlestick charts."
          },
          {
            type: "quiz",
        ...
        summary: "Technical analysis is a powerful tool for traders. As you progress, you'll learn more advanced patterns and indicators."
      };
    } else {
      return {
          ...
          {options.map((option, index) => (
            <div 
              ...
              className={`
                ...
                ${!answered ? 'border-gray-600 hover:border-blue-400' : 
                  index === correctAnswer ? 'border-green-500 bg-green-500 bg-opacity-20' :
                  index === selectedAnswer ? 'border-red-500 bg-red-500 bg-opacity-20' : 'border-gray-600'}
              `}
            >
              <div className="flex items-center justify-between">
                <span>{option}</span>
                {answered && index === correctAnswer && <Check className="h-5 w-5 text-green-500" />}
                {answered && index === selectedAnswer && index !== correctAnswer && <X className="h-5 w-5 text-red-500" />}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="w-full bg-gray-800 h-2 rounded-full mb-8">
          <div 
            ...
            style={{ width: lessonComplete ? '100%' : ${((currentStep + 1) / (content.steps.length + 1)) * 100}% }}
          ></div>
        </div>
                ...
                <>
                  <div className="mb-6">
                    ...
                    <div className="bg-gray-700 p-4 rounded-lg">
                      <p className="text-sm text-gray-400 mb-2">This is a placeholder for an interactive component.</p>
                      <p className="text-gray-300">In the full app, this would be an interactive exercise.</p>
                    </div>
                  </div>
                  <button 
                    onClick={handleNextStep}
